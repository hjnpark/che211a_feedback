...
HP: Three of them, from C_3, are obvious. The other three are tricky. 
    You can also rotate NH3 with an axis that is perpendicular to the C_3 axis.
    I think that's why rotational sym. number is 6. 
'''
sym_number = [2, 2, 6]
...
HP: You are right. In fact there is something called json that I use quite often. It's like dictionaries inside of a dictionary.
...
# HP: I modified the following two functions for the Rotational partition function 

def rotational_partition_highT_approx_linear(sigma, temp, rot_const):
    h = 6.62607004e-34 # j * s
    kb = 1.380649 * 10 ** -23 # J/K
    freq = c * rot_const * 100 * h #Hz
    #E = freq * h
    #rot_temp = E/ kb
    #T = temp
    #z = T / (sigma * rot_temp)
    z = kb*temp/(sigma*freq)
    return z
...
def rotational_partition_highT_approx_nonlinear(sigma, temp, rot_constant):
   ...
   #E1 = freq1 * h/1000 * NA # kJ/mol
    ...
    z = (np.sqrt(np.pi)/sigma) * np.sqrt(((kb*temp)**3) / (freq1 * freq2 * freq3))
    return z#, E1, E2, E3
...
# HP: Added this funciton to debug
def Keq (H, N, NH, T):
    """
    This function calculates Keq of Haber-Bosch reaction with given partition functions.
    
    Parameters
    ----------
    T : array
        An array of temperatrues
    
    H : float
        Partition function of H2
    N : float
        Partition function of N2
    NH : float
        Partition function of NH3
    Return
    ------
    keq : array
        An array of Keq.
...
# HP: You don't need to loop through the array. You can just feed it as an array, then you will get the results as an array. 
...
Keq_tr = Keq(z_trans_H2, z_trans_N2, z_trans_NH3, temps) * conc_std
Keq_rot = Keq(z_rot_H2, z_rot_N2, z_rot_NH3, temps)
Keq_vib = Keq(z_vib_H2, z_vib_N2, z_vib_NH3, temps)
Keq_el = Keq(z_el_H2, z_el_N2, z_el_NH3, temps)
...
Keq = Keq_tr * Keq_rot * Keq_vib * Keq_el
#Keq = volume * conc_std * z_NH3 / (((np.sqrt(z_N2)) * (np.sqrt(z_H2))**3))
#Keq.append()
plt.plot(temps, np.log10(Keq), label='From Partition Functions')
plt.plot(temps, haber_equil_const_empirical_fit(temps), label ='Empirical')
plt.legend()
plt.show()
'''
e)
